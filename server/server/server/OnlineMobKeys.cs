...
namespace server
{
    ...
    class OnlineMobKeys
    {
        ...
        private List<OnlinePlatform> platformsThatWereSearched;
        ...
        public OnlineMobKeys(OnlineMob mob, OnlineMap map)
        {
            this.mob = mob;
            this.map = map;
            ...
            platformsThatWereSearched = new List<OnlinePlatform>();

            right = (random.Next(0, 2) > 0) ? true : false;
            left = !right;

            indexForRandomState = 0;
            randomTime = random.Next(30, 240);
        ...
        public void Update()
        {
            ...
            if (!mob.isAttacked)
            {
                ...
                else // If mob can jump
                {
                    ...
                    if ((right && map.rightLimit - mob.position.X <= 20) ||
                        (left && mob.position.X - map.leftLimit <= 20) ||
                        (++indexForRandomState == randomTime))
                    {
                        left = !(right = !right);
            ...
            {
                OnlinePlayer player = mob.playerToFollow;

                if (mob.CanLaunchSkill && !player.isDead && random.Next(0, 50) == 7 && Vector2.Distance(mob.position, player.position) <= Skill.MAX_DISTANCE)
                {
                    skill = true;
                    return;
                }
                ...
                if (mob.canJump)
                {
                    ...
                    // If mob and player are on same platform, then mob goes to player's position
                ...
                {
                    // If mob and player are on about the same X coordinate
                    if (Math.Abs(mob.position.X - player.position.X) < mob.speed.X)
                    {
                        left = right = jump = false;
                        return;
                    }

                    // If player is left to mob, then move left
        ...
        private void FillPath(List<OnlinePlatform> platforms, OnlinePlayer player)
        {
            path.Clear();
        ...
        private bool UP_RECURSION(OnlinePlatform plat, OnlinePlayer player)
        {
            ...
            {
                thisPathRecord = SearchRightOrLeft(plat, out wasFound, plat.rightmostPart, +0.1f);
            ...
            if (mob.position.X < plat.leftmostPart.X)
            {
                if (mob.position.X < plat.leftmostPart.X)
                {
                    thisPathRecord = SearchRightOrLeft(plat, out wasFound, plat.leftmostPart, -0.1f);
                    searchedPos = thisPathRecord.destination;
        ...
        private PathRecord SearchRightOrLeft(OnlinePlatform current_plat, out bool wasFound, Vector2 platPart, float rotation_addition)
        {
        ...
        private PathRecord SearchMiddle(OnlinePlatform current_plat, out bool wasFound)
        {
            MapRecord[,] mapMask = map.mapMask;
            MapRecord record;
        ...
        private void DOWN(OnlinePlatform plat)
        {
